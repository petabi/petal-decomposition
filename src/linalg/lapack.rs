    ...
    (@body, $scalar:ty, $heev:path, $gelqf:path, $gesdd:path, $gesvd:path, $unglq:path, $($rwork_ident:ident),*) => {
        impl Lapack for $scalar {
            unsafe fn gelqf(m: i32, n: i32, mut a: &mut [Self], ld_a: i32) -> Result<Vec<Self>, i32> {
                ...
                unsafe { $gelqf(m, n, &mut a, ld_a, &mut tau, &mut work_size, -1, &mut info) };
                if info != 0 {
                    return Err(info);
                }
                ...
                unsafe { $gelqf(m, n, &mut a, ld_a, &mut tau, &mut work, lwork as i32, &mut info) };
                if info != 0 {
                    return Err(info);
                }
            ...
            unsafe fn gesdd(m: i32, n: i32, mut a: &mut [Self]) -> Result<SvdOutput<Self>, i32> {
                ...
                let mut u = unsafe { vec_uninit((n * k) as usize) };
                let mut vt = unsafe { vec_uninit((k * m) as usize) };
                ...
                let mut $rwork_ident = {
                    ...
                    unsafe { vec_uninit(lwork as usize) }
                };
                )*
            ...
            unsafe fn gesvd(jobvt: u8, m: i32, n: i32, mut a: &mut [Self]) -> Result<SvdOutput<Self>, i32> {
                ...
                let mut u = if jobvt == b'A' {
                    unsafe { vec_uninit((n * n) as usize) }
                } else {
                    Vec::new()
                };
                let mut vt = unsafe { vec_uninit((m * m) as usize) };
            ...
            unsafe fn unglq(m: i32, n: i32, k: i32, mut a: &mut [Self], ld_a: i32, tau: &[Self]) -> Result<(), i32> {
                let mut info = 0;
                let mut work_size = [Self::zero()];
                unsafe { $unglq(m, n, k, &mut a, ld_a, &tau, &mut work_size, -1, &mut info) };
                if info != 0 {
                    return Err(info);
                }
                ...
                let lwork = work_size[0].to_usize().expect("valid integer");
                let mut work = unsafe { vec_uninit(lwork) };
                unsafe { $unglq(m, n, k, &mut a, ld_a, &tau, &mut work, lwork as i32, &mut info) };
                if info != 0 {
                    return Err(info);
                }
